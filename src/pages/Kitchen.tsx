...
export default function Kitchen() {
  ...
  return <div className="mx-auto max-w-7xl px-6 py-8 sm:px-8 lg:px-10">
    ...
    {notice && <div className="mt-5 flex items-center gap-2 rounded-xl border border-success/25 bg-success/10 p-3 text-sm text-success"><LuCheck />{notice}</div>}
    ...
    <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-xl font-semibold">Active preparation queue</h2><p className="text-sm text-muted-foreground">Oldest tickets appear first.</p></div><div className="flex flex-wrap rounded-xl border bg-card p-1 shadow-sm">{(['All', 'Hot', 'Cold', 'Bakery'] as const).map((station) => <button key={station} onClick={() => setActiveStation(station)} className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${activeStation === station ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'}`}>{station}</button>)}</div></div>

    <div className="mt-4 grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]"><section>{loading ? <div className="flex min-h-72 items-center justify-center gap-2 rounded-2xl border bg-card text-sm text-muted-foreground"><LuLoaderCircle className="animate-spin" /> Loading live tickets…</div> : visibleOrders.length === 0 ? <div className="flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed bg-card text-center"><span className="flex size-14 items-center justify-center rounded-2xl bg-success/10 text-success"><LuSparkles className="size-6" /></span><h3 className="mt-4 font-semibold">Kitchen is all caught up</h3><p className="mt-1 text-sm text-muted-foreground">New POS orders will appear automatically.</p></div> : <div className="grid gap-4 lg:grid-cols-2">{visibleOrders.map((order) => <Ticket key={order.id} order={order} now={clock} working={workingId === order.id} onAdvance={() => void advance(order)} />)}</div>}</section>
      <aside className="space-y-4"><section className="rounded-2xl border bg-card p-5 shadow-sm"><div className="flex items-center gap-2"><span className="flex size-9 items-center justify-center rounded-xl bg-accent/10 text-accent"><LuPackageCheck /></span><div><h3 className="font-semibold">Products connected</h3><p className="text-xs text-muted-foreground">Live recipe availability</p></div></div><div className="mt-4 space-y-3">{(['Hot', 'Cold', 'Bakery'] as const).map((station) => <div key={station} className="flex items-center justify-between rounded-xl bg-muted/50 px-3 py-2.5"><span className="text-sm font-medium">{station} menu</span><span className="rounded-full bg-card px-2 py-0.5 text-xs font-bold text-secondary">{offerings[station].length}</span></div>)}</div></section><section className="rounded-2xl border bg-card p-5 shadow-sm"><h3 className="flex items-center gap-2 font-semibold"><LuUtensilsCrossed className="text-secondary" /> Available menu</h3><div className="scrollbar-thin mt-4 max-h-80 space-y-2 overflow-y-auto">{menu.map((item) => { const ingredients = item.ingredients.length ? item.ingredients : item.inventoryItem ? [{ quantity: 1, inventoryItem: item.inventoryItem }] : []; const low = ingredients.some((ingredient) => Number(ingredient.inventoryItem.quantity) < Number(ingredient.quantity)); return <div key={item.id} className="rounded-xl border p-3"><div className="flex items-center justify-between gap-2"><span className="text-sm font-semibold">{item.name}</span><span className={`size-2 rounded-full ${low ? 'bg-warning' : 'bg-success'}`} title={low ? 'Low ingredients' : 'Ingredients available'} /></div><p className="mt-1 text-xs text-muted-foreground">{ingredients.length ? ingredients.map((ingredient) => ingredient.inventoryItem.name).join(' · ') : 'No product recipe linked'}</p></div> })}</div></section></aside>
    </div>
  </div>
}

function Metric({ label, value, icon, tone }: { label: string; value: number; icon: React.ReactNode; tone: 'secondary' | 'warning' }) { return <div className="rounded-2xl border bg-card p-5 shadow-sm"><div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{label}</span><span className={`flex size-9 items-center justify-center rounded-xl ${tone === 'warning' ? 'bg-warning/15 text-warning' : 'bg-secondary/10 text-secondary'}`}>{icon}</span></div><p className="mt-2 font-display text-3xl font-semibold">{value}</p></div> }
